...
class EvtRepLrn(nn.Module):

    def __init__(self, config_model, config_data):
        super(EvtRepLrn, self).__init__()
        ...
        self.mask_fc = nn.Sequential(nn.Linear(config_model.hidden_dim, self.vocab_size))

    def forward(self, batch):
        evt_q, evt_q_lengths = batch.evt_q_ids, batch.evt_q_lengths
        evt_k, evt_k_lengths = batch.evt_k_ids, batch.evt_k_lengths
        evt_p1, evt_p1_lengths = batch.evt_p1_ids, batch.evt_p1_lengths
        evt_p2, evt_p2_lengths = batch.evt_p2_ids, batch.evt_p2_lengths
        evt_p3, evt_p3_lengths = batch.evt_p3_ids, batch.evt_p3_lengths
        ...
        k1, k1_outputs = self.encoder_q(evt_p1, evt_p1_lengths, is_train=True)
        k1 = nn.functional.normalize(k1, dim=1)
        
        k2, k2_outputs = self.encoder_q(evt_p2, evt_p2_lengths, is_train=True)
        k2 = nn.functional.normalize(k2, dim=1)

        k3, k3_outputs = self.encoder_q(evt_p3, evt_p3_lengths, is_train=True)
...
class EventEncoder(nn.Module):
    ...
    def __init__(self,config_model, config_data):
        ...
        self.encoder = tx.modules.BERTEncoder(hparams=self.config_model.encoder)
        self.fc = nn.Sequential(nn.Linear(config_model.hidden_dim, config_model.hidden_dim))
    ...
    def forward(self, event_ids, event_lengths, is_train=False):
        ...
        outputs, pooled_output = self.encoder(inputs=encoder_input, sequence_length=encoder_input_length)
        inputs_padding = sequence_mask(event_lengths, event_ids.size()[1]).float()
