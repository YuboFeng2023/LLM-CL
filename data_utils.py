...
def word_repetition(tokens, prob: float = 0, max_dup: int = 1):
    tmp = np.random.binomial(n=1, p=prob)
    if tmp:
        l = list()
        index = [i for i in range(len(tokens))]
        beg, end = 1, len(tokens) - 1  # skip [CLS] and [SEP]
        candidate = index[beg:end]  # select tokens of event
        max_dup = max_dup if max_dup < len(candidate) else len(candidate)  # set max feasible duplication
        dup = random.randint(a=1, b=max_dup)
        index = random.sample(candidate, k=dup)
...
def augment(target: str, backup: str):
    if len(target):
        target = map_tex_to_tokens(target)
        target_ids = tokenizer.map_text_to_id(target)
    else:
        target = map_evt_to_tokens_for_text(backup)
        target_ids = tokenizer.map_text_to_id(target)
        target_ids = word_repetition(target_ids, prob=0.5, max_dup=3)
    return target_ids
    ...
    def __iter__(self) -> Iterator[List[str]]:
        ...
        with self._open_file(path) as f:
            for line in f:
                ...
                evt_q = ex['evt_q']
                evt_k = evt_q
                evt_p1 = ex['evt_k'][0] if len(ex['evt_k']) else str()
                evt_p2 = ex['evt_k'][1] if len(ex['evt_k']) else str()
                evt_p3 = ex['evt_k'][2] if len(ex['evt_k']) else str()
                yield evt_q, evt_k, evt_p1, evt_p2, evt_p3
...
class TrainData(tx.data.DatasetBase[Example, Example]):
    ...
    def process(self, raw_example):
        ...
        evt_q, evt_k_str = raw_example[0], raw_example[1]
        evt_p1, evt_p2, evt_p3 = raw_example[2], raw_example[3], raw_example[4]

        evt_q = map_evt_to_tokens_for_text(evt_q)
        ...
        evt_k = map_evt_to_tokens_for_text(evt_k_str)
        evt_k_ids = tokenizer.map_text_to_id(evt_k)

        evt_p1_ids = augment(evt_p1, evt_k_str)
        evt_p2_ids = augment(evt_p2, evt_k_str)
        evt_p3_ids = augment(evt_p3, evt_k_str)

        return {
            # "evt_k": ' '.join(evt_k_str),
            "evt_q_ids": evt_q_ids,
            "evt_k_ids": evt_k_ids,
            "evt_p1_ids": evt_p1_ids,
            "evt_p2_ids": evt_p2_ids,
            "evt_p3_ids": evt_p3_ids,
            "mask_pos": mask_pos,
            "mask_id": mask_id,
            # "evt_freq": evt_freq,
        }
    ...
    def collate(self, examples: List[Example]) -> tx.data.Batch:
        ...
        evt_q_ids, evt_q_lengths = tx.data.padded_batch([ex["evt_q_ids"] for ex in examples], pad_value=pad_token_id)

        # evt_k = [ex["evt_k"] for ex in examples]
        evt_k_ids, evt_k_lengths = tx.data.padded_batch([ex["evt_k_ids"] for ex in examples], pad_value=pad_token_id)

        evt_p1_ids, evt_p1_lengths = tx.data.padded_batch([ex["evt_p1_ids"] for ex in examples], pad_value=pad_token_id)
        evt_p2_ids, evt_p2_lengths = tx.data.padded_batch([ex["evt_p2_ids"] for ex in examples], pad_value=pad_token_id)
        evt_p3_ids, evt_p3_lengths = tx.data.padded_batch([ex["evt_p3_ids"] for ex in examples], pad_value=pad_token_id)
        ...
        return tx.data.Batch(
            ...
            evt_p1_ids=torch.from_numpy(evt_p1_ids),
            evt_p1_lengths=torch.tensor(evt_p1_lengths),
            evt_p2_ids=torch.from_numpy(evt_p2_ids),
            evt_p2_lengths=torch.tensor(evt_p2_lengths),
            evt_p3_ids=torch.from_numpy(evt_p3_ids),
            evt_p3_lengths=torch.tensor(evt_p3_lengths),
